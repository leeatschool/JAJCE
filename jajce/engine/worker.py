...
class BatchWorker(QThread):
    ...
    def run(self):
        ...
        try:
            for idx, path_str in enumerate(self.items):
                ...
                if result.success:
                    ...
                    self.log_message.emit(
                        f"Converted ({idx+1}/{total}): {result.output_path} | "
                        f"Saved: {result.compression_ratio:+.1f}% | "
                        f"Tags: {len(result.tags)}{note_str}"
                    )
                else:
                    error_count += 1
                    self.log_message.emit(f"Error converting ({idx+1}/{total}) {path_str}: {result.error_message}")
            ...
            self.log_message.emit(f"Critical error in batch worker: {e}")
        finally:
